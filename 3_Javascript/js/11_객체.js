...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    const product = { 
        ...
        information : function(){
            ...
            // 현재 객체를 뜻하는 this를 앞에 붙여야 된다! (this == 현재 객체)
    ...
    // 결과 출력
    div1.innerHTML=""; //div1 안에 있는 내부 내용 모두 삭제
    ...
    div1.innerHTML += "<hr>"; // 선긋기
    
    
    // 자바스크립트 객체용 향상된 for문 (for in)
    // -> 객체 내부에 작성된 key를 순서대로 하나씩 꺼내옴
    for(let key in product){
        //div1.innerHTML+=product.key + "<br>";//undefined : key 값이 존재하지 않는다 
...
function Student(name,grade,ban){
    ...
    // 속성 
    // this == 생성되는 객체
    this.name=name; // 생성되는 객체 name에 매개변수 name대입 
    this.grade=grade; // 생성되는 객체 grade에 매개변수 grade대입 
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    const std3 = new Student("홍길래",1,9);
